...
describe 'Users::OmniauthCallbacks', type: :request do
  describe '#github' do
    ...
    context "when a git hub user doesn't have email" do
      before do
        OmniAuth.config.test_mode = true
        OmniAuth.config.mock_auth[:github] = OmniAuth::AuthHash.new({
                                                                      provider: 'github',
  ...
  describe '#google' do
    context 'when a google user has an email' do
      before do
        OmniAuth.config.test_mode = true
      ...
      it { expect(response).to render_template(:home) }
      it { expect(response.body).to include('Successfully authenticated from Google account.') }
    end

    context "when a google user doesn't have email" do
      before do
        OmniAuth.config.test_mode = true
        OmniAuth.config.mock_auth[:google_oauth2] = OmniAuth::AuthHash.new({
                                                                             provider: 'google',
      ...
      it {
        expect(response.body)
          .to include('Could not authenticate you from Google because &quot;Email can&#39;t be blank&quot;.')
      }
    end
    ...
    context 'when invalid credentials' do
      ...
      it {
        expect(response.body)
          .to include('Could not authenticate you from GoogleOauth2 because &quot;Invalid credentials&quot;.')
      }
    end
  end
end
